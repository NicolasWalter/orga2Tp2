    ...
    for (i; i < 16; i++)
    {
        ...
        for (j; j < 12; j++)
        {
           ...
           /* if(!(i<2 || j<2 || i+2 >= filas || j+2 >= cols) ){
                ...
                printf("%d %d %d %d %d\n",i ,j, ((bgra_t*) &src_matrix[i][(j) * 4])->r,((bgra_t*) &src_matrix[i][(j) * 4])->g,((bgra_t*) &src_matrix[i][(j) * 4])->b);
                ...
                (*p_d).b=MIN(MAX(ldrB,0),255);
                            ...
                            printf("%d %d %d %s",((bgra_t*) &src_matrix[i][(j) * 4])->r,((bgra_t*) &src_matrix[i][(j) * 4])->g,((bgra_t*) &src_matrix[i][(j) * 4])->b,"-");
